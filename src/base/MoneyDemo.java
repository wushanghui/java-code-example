...
//        // 等额本息还款公式推导 设贷款总额为A，银行月利率为b，总期数为m（个月），月还款额设为X
//        BigDecimal a = new BigDecimal("920000");
//        BigDecimal b = new BigDecimal("5.635").divide(new BigDecimal("100")).divide(new BigDecimal("12"), 10, BigDecimal.ROUND_HALF_UP);
//        BigDecimal x = a.multiply(b).multiply(new BigDecimal("1").add(b).pow(360)).divide(new BigDecimal("1").add(b).pow(360).subtract(new BigDecimal("1"))).setScale(2, BigDecimal.ROUND_HALF_UP);
...
//    }
     ...
     */
    public static void main(String[] args) {
        double invest = 904780.74; // 本金
        int month = 345;
        // 5.635 - 0.15 = 5.485
        // 5301
        double yearRate = 0.05485; // 年利率
